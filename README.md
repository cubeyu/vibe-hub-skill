...
选择一个方向后，Agent 会先给出短路线。等你明确当前要学的主题，Agent 会自己判断文字、课程页还是互动更适合理解，不需要你学习怎样指挥 Skill。
...
互动默认运行在本机。当你明确要学习某个主题，而且页面确实有助于理解时，Agent 会自行使用内置浏览器打开课程或互动，不要求你先说“打开页面”。第一次使用、推荐方向和普通项目修复不会自动打开页面。
